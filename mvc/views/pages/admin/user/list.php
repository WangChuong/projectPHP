...
<table class="table table-striped table_user">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="align-middle "><img src="<?php echo _PATH_AVATAR . $user['avatar'] ?>"
                    class="align-middle img-thumbnail"
                    style="width: 80px; height: 80px; margin-top: 5px; max-width: 100%; object-fit: cover; object-position: center;">
            </td>
            <td class="align-middle" style="width: 100px;">
                <?php echo '<p class="m-0">' . $user['phone'] . '</p>' . '<p class="m-0">' . $user['email'] . '</p>' . '<p>' . $user['address'] . '</p>' ?>
            </td>
            <td class="align-middle "><?php echo $user['created_at'] ?></td>
            <td class="align-middle "><?php echo $user['updated_at'] ?></td>

            <td class="align-middle text-center ">
                <a class="align-middle btn btn-outline-primary"
                    href="<?php echo _WEB_ROOT . '/user/update_user/' . $user['id'] ?>">
                    <i class="align-middle fas fa-edit"></i>
                </a>
                <a class="align-middle handle_delete btn btn-outline-danger"
                    href="<?php echo _WEB_ROOT . '/user/delete_user/' . $user['id'] ?>">
