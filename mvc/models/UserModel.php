...
class UserModel extends DB
{
    ...
    function insert($name, $avatar, $email, $password, $phone, $address, $desc, $create_at)
    {
        $insert = "INSERT INTO users(name, avatar, gr_id, email, password, phone, address, description, created_at) VALUE('$name', '$avatar' ,'$email', '$password', '$phone', '$address', '$desc', '$create_at')";
        return $this->pdo_execute($insert);
    }

    function updateUser($id, $name, $avatar,  $email, $password, $phone, $address, $desc, $updated_at)
    {
        if (!empty($password) && empty($avatar)) {
            $update = "UPDATE users SET name = '$name', email = '$email', password = '$password', phone = '$phone', address = '$address', description = '$desc', updated_at = '$updated_at' WHERE id = '$id'";
            ...
            $update = "UPDATE users SET name = '$name', avatar = '$avatar', email = '$email', password = '$password', phone = '$phone', address = '$address', description = '$desc', updated_at = '$updated_at' WHERE id = '$id'";
        }
